...
namespace OwinWebListenerHost.RequestProcessing
{
    ...
    internal sealed class ResponseHeadersDictionary : HeadersDictionaryBase
    {
        ...
        internal ResponseHeadersDictionary(Response response)
            : base()
        {
            _response = response;
        ...
        private bool HasContentLength => _response.ContentLength.HasValue;

        private string[] ContentLength => new[]
        {
            _response.ContentLength.Value.ToString(CultureInfo.InvariantCulture)
        };
        ...
        protected override void Set(string header, string value)
        {
            ...
            if (header.Equals(Constants.ContentLengthHeader, StringComparison.OrdinalIgnoreCase))
            {
                _response.ContentLength = long.Parse(value, CultureInfo.InvariantCulture);
            }
        ...
        public override IEnumerator<KeyValuePair<string, string[]>> GetEnumerator()
        {
            ...
            {
                yield return new KeyValuePair<string, string[]>(key, Headers.GetValues(key).ToArray());
            }
        }
    }
}
